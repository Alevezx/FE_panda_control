...
void collisionCheckerThread(const pinocchio::Model& model, std::ofstream& outfile_dist) {
    ...
    while (!trajectory_done.load()) {
        ...
        if (!skeleton.empty()) {
            ...
            outfile_dist << "\t\t<point id='0'>" << closest.C_r.x() << " "
                          << closest.C_r.y() << " " << closest.C_r.z() << "</point>\n";
            outfile_dist << "\t\t<point id='1'>" << closest.C_h.x() << " "
                          << closest.C_h.y() << " " << closest.C_h.z() << "</point>\n";
...
void executeTrajectory (
    ...
    const double TRAJ_STEP = 0.001;  // matches franka_sim's fixed 1kHz control period
    ...
    //std::ofstream outfile_quintic("../logs/quintic.txt", std::ios::app);
    ...
    while (flag_continue) {
        ...
        double t = 0.0;  // accumulated from period.toSec(), matches real libfranka convention
        try {
            robot.control([&](const franka_sim::RobotState& state,
                            franka_sim::Duration period) -> franka_sim::JointPositions
            {
                t += period.toSec();
                ...
                if (stop_flag.load())
                {
                    ...
                    for (int j = 0; j < 7; ++j){
                        q_meas(j) = state.q_d[j];
                        q_p_local(j) = state.dq[j];
                        q_pp_local(j) = state.ddq_d[j];
                    }
                    return franka_sim::MotionFinished(
                        franka_sim::JointPositions(state.q_d));
                }
        ...
        if (stop_flag_local && do_stop) {
            ...
            // Precompute the braking trajectory once — q_meas, q_p_local,
            // q_pp_local, and stop_duration are all fixed for the duration
            // of this maneuver, so there's no need to rebuild the quintic
            // for all 7 joints on every single 1ms control tick.
            int stop_num_points = static_cast<int>(stop_duration / TRAJ_STEP) + 1;
            Eigen::MatrixXd Q_stop(stop_num_points, 7);
            for (int j = 0; j < 7; ++j) {
            ...
            double t = 0.0;  // accumulated across ticks, not reset every call
            ...
            try {
                                ...
                                franka_sim::Duration period) -> franka_sim::JointPositions
                {
                    ...
                    int idx = std::min(static_cast<int>(t / TRAJ_STEP),
                                        stop_num_points - 1);

                    std::array<double, 7> q_cmd;
                    for (int j = 0; j < 7; ++j){
                        q_cmd[j] = Q_stop(idx, j);
                        q_meas(j) = state.q_d[j];
                    }
                    ...
                    return franka_sim::JointPositions(q_cmd);
                });
            } catch (const franka_sim::Exception& e) {
                std::cerr << "executeTrajectory: robot fault during main motion: " << e.what() << "\n";
                robot.automaticErrorRecovery();
...
void executeTask(
    ...
    "../skeleton/skeleton_coords_fixed.xml");
...
int main(int argc, char** argv) {    
    ...
    std::cin.get();
    try {
        franka_sim::Robot robot(ROBOT_IP);
        ...
        robot.control([&](const franka_sim::RobotState& state,
            ...
            if (time > move_duration) {
                return franka_sim::MotionFinished(franka_sim::JointPositions(q_cmd));
            }
            return franka_sim::JointPositions(q_cmd);
        });
    ...
    } catch (const franka_sim::Exception& ex){
