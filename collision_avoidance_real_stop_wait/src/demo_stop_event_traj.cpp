...
void executeTrajectory (
    ...
    // logica a stati del movimento
    enum State {RUNNING, STOPPING};
        ...
        try {
                            ...
                            franka::Duration period) -> franka::JointPositions
            {
                ...
                // Update shared robot state for collision checker 
                {
                    std::lock_guard<std::mutex> lock(robot_state_mutex);
                    for (int j = 0; j < 7; ++j) {
                        shared_robot_state.q[j]   = state.q_d[j];
                        shared_robot_state.q_p[j] = state.dq_d[j];
                ...
                // Store measured position, speed and acceleration
                for (int j = 0; j < 7; ++j){
                    q_meas(j) = state.q_d[j];
                    q_p_local(j) = state.dq_d[j];
                ...
                if (current_state == RUNNING && stop_flag.load()) {
                    ...
                    std::cout << "Stop detected, computed poly5, stop duration = " << stop_duration
                                << " num points = " << stop_num_points << "\n";
                ...
                if (current_state == STOPPING) {
                    ...
                    std::cout << "Sending braking traj, " << stop_idx << " out of " << stop_num_points << "\n";
                    return franka::JointPositions(std::array<double,7>{Q_stop(stop_idx,0), Q_stop(stop_idx,1), Q_stop(stop_idx,2),
                                        Q_stop(stop_idx,3), Q_stop(stop_idx,4), Q_stop(stop_idx,5), Q_stop(stop_idx,6)});
                } 
...
void executeTask(
    ...
    "../skeleton/skeleton_coords_moving.xml");
