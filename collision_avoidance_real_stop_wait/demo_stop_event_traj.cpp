                            ...
                            franka::Duration period) -> franka::JointPositions
            {
                ...
                if (stop_flag.load() || is_braking) {
                    if(!is_braking) {
                        ...
                        for (int j = 0; j < 7; ++j) {
                            q_0_brake(j) = state.q_d[j];
                            v_0_brake(j) = state.dq_d[j];
                            a_0_brake(j) = state.ddq_d[j];
                        }
                        t_brake = 0.0;
                    }
                    ...
                    int idx = std::min(static_cast<int>(t_brake / TRAJ_STEP), stop_num_points - 1);

                    std::cout << "Sending braking traj\n";
                    return franka::JointPositions(std::array<double,7>{Q_stop(idx,0), Q_stop(idx,1), Q_stop(idx,2),
                                        Q_stop(idx,3), Q_stop(idx,4), Q_stop(idx,5), Q_stop(idx,6)});
                }
