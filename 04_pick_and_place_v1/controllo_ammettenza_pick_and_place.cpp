...
int main(int argc, char** argv) {
  ...
  // --- DEFINIZIONE WAYPOINTS (A, B, C, D, E, F) ---
  // Coordinate in metri [x, y, z]
  std::vector<Eigen::Vector3d> waypoints;
  // A (Home/Start)
  waypoints.push_back(Eigen::Vector3d(0.3, 0.5, 0.5));  // Index 0: A
  // B
  waypoints.push_back(Eigen::Vector3d(0.3, 0.5, 0.2)); // Index 1: B
  // C
  waypoints.push_back(Eigen::Vector3d(0.5, 0.0, 0.5));  // Index 2: C
  // D
  waypoints.push_back(Eigen::Vector3d(0.5, 0.0, 0.2)); // Index 3: D
  // E
  waypoints.push_back(Eigen::Vector3d(0.3, -0.5, 0.5));  // Index 4: E
  // F
  waypoints.push_back(Eigen::Vector3d(0.3, -0.5, 0.2)); // Index 5: F
  // G: per andare su in modo che non vada vicino alla singolarità
  waypoints.push_back(Eigen::Vector3d(0.5, 0.0, 0.4)); // Index 6: G

  // --- DEFINIZIONE SEQUENZA ---
  // Pattern: A->B->A->C->D->C->E->F->E
  // Nota: Si assume che si parta già da A. Quindi il primo target è B.
  // Alla fine del ciclo E, aggiungo un ritorno ad A per ricominciare pulito il ciclo successivo.
  //std::vector<int> pattern = {1, 0, 2, 3, 2, 4, 5, 4, 6, 0}; 
  ...
  // Tempo per ogni singolo segmento (es. da A a B)
  const double segmentDuration = 4.0; 
  int currentSequenceIndex = 0;
  double segmentTime = 0.0; // Tempo locale del segmento corrente

  
  // --- ROBOT: capsule (raggio fisico per link) ---
  std::vector<double> robot_capsule_radius = {0.10, 0.10, 0.10, 0.10};
  ...
  safetyParams.r_inner = 0.10; 
  safetyParams.r_outer = 0.09; 
  safetyParams.T_stop = 0.4; // valore scelto sulla base di test empirici

  const double forceFilterGain = 0.02;   
  ...
  const double recoveryDuration = 3.0; 
  double frozenTime = 0.0; // Congela il segmentTime
  ...
  Eigen::Vector3d endPos = Eigen::Vector3d::Zero(); // Target del segmento corrente
  ...
  try {
    ...
    // --- MOVIMENTO AL PUNTO DI PARTENZA (A) ---
    // Usiamo il primo punto della lista waypoints (Index 0 = A) come start
    Eigen::Vector3d startPos_desired = waypoints[4];
    ...
    robot.control([&](const franka::RobotState& state,
      ...
      // --- INIZIALIZZAZIONE PRIMO SEGMENTO ---
      if (firstRun) {
      ...
      if (currentState == RUNNING) {
        ...
        // REPULSIONE
          ...
          // MODIFICA: Decadimento naturale del filtro invece di reset brusco
          // Il target è zero, ma lasciamo che il filtro ci arrivi gradualmente
          filteredRepulsiveForce = (1.0 - forceFilterGain) * filteredRepulsiveForce; 
        }
      } 
      // GESTIONE STATI NON RUNNING
  ...
  save_log_to_csv(log_data, "log_capsule_success_seq.csv");
  return 0;
}