...
A partire dalle immagini acquisite, la posa dello scheletro umano viene stimata mediante l'architettura a rete neurale YOLOv8 \cite{Qyrani2026, reis2023real}, i punti salienti (\textit{keypoints}) estratti vengono successivamente salvati su file di testo.
...
Quando una capsula associata all'operatore entra in contatto con una capsula \textit{soft} del robot, il sistema calcola una forza virtuale repulsiva di tipo elastico. L'entità di tale forza è proporzionale alla penetrazione tra le capsule ed è modulata mediante una funzione di raccordo continuo (\textit{smoothstep}) \cite{Qyrani2026}.

La forza risultante viene convertita nella variazione di posa cartesiana dell meccano-struttura (\textit{end-effector}) per il passo di controllo successivo, implementando il paradigma del controllo di ammettenza esplicitato nella Sezione~\ref{subsec:ammettenza}. In particolare, la dinamica dell'interazione viene modellata secondo il classico sistema sistema massa-molla-smorzatore: 

\begin{equation}
	\mathbf{M} \mathbf{\ddot{x}}(t) + \mathbf{D} \mathbf{\dot{x}}(t) + \mathbf{K} (\mathbf{x}(t) - \mathbf{x_0}) = \mathbf{F}_{ext}(t)
\end{equation}

dove $M$, $D$ e $K$ rappresentano rispettivamente le matrici di massa, smorzamento e rigidità virtuali, $x(t)$ è la posa cartesiana dell'\textit{end-effector} e $F_{ext}(t)$ indica la forza repulsiva risultante generata dal modello a capsule.

Oltre ai volumi esterni, il modello integra internamente delle capsule \textit{hard}, dimensionate ed ottimizzate con la stessa metodologia descritta nella Sezione~\ref{sec:frenata}. Nel caso in cui si verifichi una violazione di tali volumi interni, il controllore interrompe l'esecuzione e comanda una frenata morbida del robot nell'intervallo di tempo ottimizzato $T_{\text{stop}}$.
