...
\begin{equation}
\boldsymbol{M} \big(\ddot{\boldsymbol{x}}_r - \ddot{\boldsymbol{x}}_d\big) + \boldsymbol{D} \big(\dot{\boldsymbol{x}}_r - \dot{\boldsymbol{x}}_d\big) + \boldsymbol{K} \big(\boldsymbol{x}_r - \boldsymbol{x}_d\big) = \boldsymbol{F}_{\text{rep}}
\label{eq:admittance_dynamics}
\end{equation}

dove:
\begin{itemize}
	\item $\boldsymbol{x}_d, \dot{\boldsymbol{x}}_d, \ddot{\boldsymbol{x}}_d \in \mathbb{R}^3$ indicano le grandezze cartesiane desiderate (posizione, velocità, accelerazione) pianificate dal task nominale. Lo spazio $\mathbb{R}^3$ viene adottato per descrivere le 3 componenti traslazionali dell'organo terminale;
	\item $\boldsymbol{x}_r, \dot{\boldsymbol{x}}_r, \ddot{\boldsymbol{x}}_r \in \mathbb{R}^3$ rappresentano lo stato di riferimento modificato che viene calcolato online;
	\item $\boldsymbol{M}, \boldsymbol{D}, \boldsymbol{K} \in \mathbb{R}^{3 \times 3}$ costituiscono le matrici simmetriche e definite positive di massa, smorzamento e rigidezza virtuali desiderate.
\end{itemize}

\subsubsection{Calcolo online della posizione modificata}

Definendo lo spostamento relativo rispetto alla traiettoria nominale come $\boldsymbol{e}_x(t) = \boldsymbol{x}_r(t) - \boldsymbol{x}_d(t)$, la dinamica continua dell'ammettenza~(\ref{eq:admittance_dynamics}) espressa nel dominio della trasformata di Laplace si scrive come:
\begin{equation}
\Big( \boldsymbol{M} s^2 + \boldsymbol{D} s + \boldsymbol{K} \Big) \boldsymbol{E}_x(s) = \boldsymbol{F}_{\text{rep}}(s)
\end{equation}

da cui si ricava la funzione di trasferimento continua dell'ammettenza $\boldsymbol{G}(s) \in \mathbb{C}^{3 \times 3}$:
\begin{equation}
\boldsymbol{E}_x(s) = \boldsymbol{G}(s) \, \boldsymbol{F}_{\text{rep}}(s) = \Big( \boldsymbol{M} s^2 + \boldsymbol{D} s + \boldsymbol{K} \Big)^{-1} \boldsymbol{F}_{\text{rep}}(s)
\end{equation}

Per l'implementazione del controllo in tempo reale al periodo di campionamento $T_s$ (pari a $1\text{ ms}$ per un ciclo a $1\text{ kHz}$), si ottiene il sistema a tempo discreto equivalente applicando la trasformazione bilineare di Tustin:
\begin{equation}
s \approx \frac{2}{T_s} \frac{1 - z^{-1}}{1 + z^{-1}}
\end{equation}

Sostituendo l'approssimazione di Tustin in $\boldsymbol{G}(s)$, si ricava la funzione di trasferimento a tempo discreto $\boldsymbol{G}_d(z)$:
\begin{equation}
\boldsymbol{E}_x(z) = \boldsymbol{G}_d(z) \, \boldsymbol{F}_{\text{rep}}(z) = \boldsymbol{G}\left( \frac{2}{T_s} \frac{1 - z^{-1}}{1 + z^{-1}} \right) \boldsymbol{F}_{\text{rep}}(z)
\end{equation}

Antitrasformando $\boldsymbol{G}_d(z)$ nel dominio del tempo discreto, si ottiene un'equazione alle differenze che permette di calcolare ricorsivamente lo scostamento $\boldsymbol{e}_x[k]$ al generico passo di campionamento $k$, in funzione dei campioni correnti e passati della forza repulsiva $\boldsymbol{F}_{\text{rep}}$ e degli stati passati dello scostamento:
\begin{equation}
\boldsymbol{e}_x[k] = \boldsymbol{\mathcal{A}}_1 \boldsymbol{e}_x[k-1] + \boldsymbol{\mathcal{A}}_2 \boldsymbol{e}_x[k-2] + \boldsymbol{\mathcal{B}}_0 \boldsymbol{F}_{\text{rep}}[k] + \boldsymbol{\mathcal{B}}_1 \boldsymbol{F}_{\text{rep}}[k-1] + \boldsymbol{\mathcal{B}}_2 \boldsymbol{F}_{\text{rep}}[k-2]
\end{equation}

dove le matrici di coefficienti $\boldsymbol{\mathcal{A}}_i$ e $\boldsymbol{\mathcal{B}}_j$ dipendono unicamente dai parametri fisici $(\boldsymbol{M}, \boldsymbol{D}, \boldsymbol{K})$ e dal tempo di campionamento $T_s$.

La posizione cartesiana di riferimento modificata al passo $k$ viene quindi calcolata sommando lo scostamento ottenuto alla posizione nominale pianificata:
\begin{equation}
\boldsymbol{x}_r[k] = \boldsymbol{x}_d[k] + \boldsymbol{e}_x[k]
\end{equation}

Il comportamento dinamico del robot durante la fase di evasione è governato dai parametri delle matrici virtuali:
\begin{itemize}
	\item Azione della massa ($\boldsymbol{M}$): filtra i picchi improvvisi della forza repulsiva, garantendo accelerazioni fluide ed evitando scatti bruschi.
	\item Azione dello smorzamento ($\boldsymbol{D}$): dissipa l'energia del sistema virtuale e previene l'insorgere di oscillazioni indesiderate durante l'allontanamento dall'operatore.
	\item Azione della rigidezza ($\boldsymbol{K}$): agisce come un richiamo elastico verso la traiettoria nominale $\boldsymbol{x}_d[k]$. Nel momento in cui l'operatore esce dalla capsula \textit{soft}, la forza repulsiva $\boldsymbol{F}_{\text{rep}}[k]$ si annulla e il termine $\boldsymbol{K}$ riporta gradualmente e in modo fluido il robot sul percorso originariamente pianificato.
\end{itemize}

La posizione cartesiana $\boldsymbol{x}_r[k]$, così calcolata, rappresenta quindi il nuovo riferimento cinematico generato dall'ammettenza, in grado di deviare localmente la traiettoria nominale per garantire l'evasione dall'operatore.
